"""Suurballe FAST_FAILOVER OpenFlow controller.

Computes two edge-disjoint shortest paths using Suurballe's algorithm, then
installs an OpenFlow FAST_FAILOVER group on the ingress switch. The dataplane
automatically switches to the backup next hop when the primary port goes down.

Complexity:   O((V+E) log V) per (src, dst) pair
Metric:       hop count (optional weights supported by the algorithm)
Multipath:    two disjoint paths (primary + backup)
ECMP:         no (failover, not load balancing)
"""
...
FF_FLOW_COOKIE = 0x5355464600000001   # "SUFF" in hex
FF_FLOW_COOKIE_MASK = 0xFFFFFFFFFFFFFFFF
FF_FLOW_PRIORITY = 100
GROUP_ID_SPACE = 0x7FFFFFFF


class SuurballeFastFailoverSwitch(SPFBaseController):
    """Edge-disjoint primary/backup routing with FAST_FAILOVER groups."""

    FLOW_COOKIE = FF_FLOW_COOKIE

    def __init__(self, *args, **kwargs):
        super(SuurballeFastFailoverSwitch, self).__init__(*args, **kwargs)
        self.path_cache = {}
        self.flow_groups = {}

    # ---------------------------------------------------------------------
    # FAST_FAILOVER group management
    # ---------------------------------------------------------------------
    ...
    def _install_group_flow(self, datapath, in_port, src_mac, dst_mac, group_id):
        ...
        datapath.send_msg(parser.OFPFlowMod(
            datapath=datapath,
            cookie=FF_FLOW_COOKIE,
            cookie_mask=FF_FLOW_COOKIE_MASK,
            command=ofproto.OFPFC_DELETE_STRICT,
            out_port=ofproto.OFPP_ANY,
            out_group=ofproto.OFPG_ANY,
            priority=FF_FLOW_PRIORITY,
            match=match,
        ))
        datapath.send_msg(parser.OFPFlowMod(
            datapath=datapath,
    ...
    def _install_fast_failover_group(self, datapath, group_id, primary_port, backup_port):
        ...
        self.logger.debug("[FF-GROUP] s%d group=%d ports=%s",
                          datapath.id, group_id, ports)
        return True

    def _delete_group(self, datapath, group_id):
        parser = datapath.ofproto_parser
        ofproto = datapath.ofproto
        datapath.send_msg(parser.OFPGroupMod(
            datapath=datapath,
    ...
    def _delete_failover_groups(self, datapath):
        parser = datapath.ofproto_parser
        ofproto = datapath.ofproto
    ...
    def install_failover(self, paths, src_mac, dst_mac):
        if not paths:
            return
        ...
        if ingress_dp is None:
            self.logger.warning("[FF-INSTALL] s%d datapath unavailable", ingress_dpid)
            return
        ...
        if len(paths) == 1:
            ...
            if existing:
                ...
                if dp:
                    self._delete_group(dp, existing[1])
        ...
        if self._install_fast_failover_group(ingress_dp, group_id, primary_port, backup_port):
            self.flow_groups[key] = (ingress_dpid, group_id)
            self._install_group_flow(ingress_dp, ingress_in_port, src_mac, dst_mac, group_id)
            self.logger.info("[FF-INSTALL] %s->%s paths=%d ingress=s%d group=%d",
    ...
    def _flush_all_flows(self):
        for dp in self.datapaths.values():
            ...
            self._delete_failover_groups(dp)
    ...
    def _reinstall_all_known_routes(self):
        ...
        for src_mac in hosts:
            for dst_mac in hosts:
                ...
                if paths:
                    self.install_failover(paths, src_mac, dst_mac)
                    installed += 1
                else:
                    unreachable += 1
        self.logger.info(
            "[TOPO] failover refresh: installed=%d skipped=%d unreachable=%d hosts=%d",
    ...
    def _packet_in_handler(self, ev):
        ...
        if dst in self.mymacs:
            ...
            if key in self.installed_paths:
                ...
                if paths:
                    self.install_failover(paths, src, dst)
                else:
                    self.logger.warning("[PKT-DROP] %s->%s: no failover path", src, dst)
                    return
            ...
            self.logger.warning("[PKT-DROP] %s->%s: s%d not in failover path", src, dst, dpid)
        else:
    ...
    def stop(self):
        self.path_cache.clear()
        self.flow_groups.clear()
        super(SuurballeFastFailoverSwitch, self).stop()
...
if __name__ == '__main__':
    ...
    sys.argv = ['suurballe_fast_failover_osken_controller', *passthrough_args, current_file]
