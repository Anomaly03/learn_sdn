...
class DijkstraSwitch(app_manager.OSKenApp):
    ...
    def _packet_in_handler(self, ev):
        ...
        # Prefer access ports but don't strictly require it for learning
        if src not in self.mymacs or self.mymacs[src][0] != dpid:
            is_access = self._is_access_port(dpid, in_port)
            if is_access or src not in self.mymacs:  # Always learn from access ports, or if first time seeing this MAC
                self._update_host_location(src, dpid, in_port)
                if not is_access and src not in self.mymacs:
                    self.logger.debug("[HOST-LEARN-FALLBACK] %s learned on switch s%d port %d (not marked as access port)",
                                     src, dpid, in_port)
    ...
    def get_topology_data(self, ev):
        ...
        # In mininet with 6 hosts and 3 switches, each switch has 4-5 ports (1-2 for hosts, 2-3 for inter-switch)
        # Assume max port 10 for simplicity (covers mininet topologies)
        self.access_ports = defaultdict(set)
        MAX_PORT = 10
        for switch in switch_list:
            ...
            self.logger.debug("[PORT-MAP] s%d: inter_switch_ports=%s, access_ports=%s",
                             dpid, sorted(inter_ports), sorted(access))
